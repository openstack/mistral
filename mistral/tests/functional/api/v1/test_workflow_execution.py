...
class MistralWorkflowExecutionTests(base.TestCaseAdvanced):
    ...
    def test_reverse_flow(self):
        ...
        ex = self.client.create_execution_wait_success(
            'test', CONTEXT, 'send_greeting', 150)[1]

        task = self.client.get_task_by_name(
            'test', ex['id'], 'build_full_name')
        task = json.loads(task['output'])
        self.assertEqual(task['task']['build_full_name']['string'],
                         "John Doe")

        task = self.client.get_task_by_name(
            'test', ex['id'], 'build_address')
        task = json.loads(task['output'])
        self.assertEqual(task['task']['build_address']['string'],
                         "To John Doe")

        task = self.client.get_task_by_name(
            'test', ex['id'], 'build_greeting')
        task = json.loads(task['output'])
        self.assertEqual(task['task']['build_greeting']['string'],
                         "Dear John Doe")

        task = self.client.get_task_by_name(
            'test', ex['id'], 'send_greeting')
        task = json.loads(task['output'])
    ...
    def test_task_with_two_dependencies(self):
        self.client.upload_workbook_definition(
            'test', 'resources/data_flow/task_with_two_dependencies.yaml')
    ...
    def test_direct_flow_tasks_on_success(self):
        self.client.upload_workbook_definition(
            'test', 'resources/data_flow/three_subsequent_tasks.yaml')

        ex = self.client.create_execution_wait_success(
            'test', CONTEXT, 'build_full_name', 150)[1]
    ...
    def test_two_dependent_tasks(self):
        self.client.upload_workbook_definition(
            'test', 'resources/data_flow/two_dependent_tasks.yaml')

        ex = self.client.create_execution_wait_success(
            'test', CONTEXT, 'build_greeting', 150)[1]
    ...
    def test_two_subsequent_tasks(self):
        self.client.upload_workbook_definition(
            'test', 'resources/data_flow/two_subsequent_tasks.yaml')
