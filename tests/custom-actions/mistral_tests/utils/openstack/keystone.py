...
def get_session_and_auth(_context, **kwargs):
    ...
    project_endpoint = get_endpoint_for_project(ctx=_context, **kwargs)
    endpoint = format_url(
        project_endpoint.url,
        {
            'tenant_id': _context.project_id,
            'project_id': _context.project_id
        }
    )

    auth = Token(endpoint=endpoint, token=_context.auth_token)

    auth_uri = _context.auth_uri or CONF.keystone_authtoken.auth_uri
    ks_auth = Token(
        endpoint=auth_uri,
        token=_context.auth_token
    )
    session = ks_session.Session(
        auth=ks_auth,
        verify=_determine_verify(_context)
    )
...
def get_keystone_endpoint_v2(ctx=None):
