...
from mistral_tests.utils import inspect_utils as i_u
...
def get_mapping():
    ...
    mapping_file_path = os.path.join(dir, '..', path)

    LOG.info(
        "Proc   essing OpenStack action mapping from file: %s",
        mapping_file_path
    )
...
class OpenStackActionGenerator(object):

    @classmethod
    def prepare_action_inputs(cls, origin_inputs):
    ...
    def create_action_class(cls, method_name):
        ...
        action_class = type(str(method_name), (cls.get_base_action_class(),),
    ...
    def create_actions(cls):
        LOG.info("-------------log-----------create_act")
        mapping = get_mapping()
        method_dict = mapping.get(cls.get_action_namespace(), {})
        ...
        for action_name, method_name in method_dict.items():
            ...
            try:
                ...
                LOG.exception(
                    ...
                    cls.get_action_namespace(), action_name
                )
                continue
            ...
            if cls.get_action_namespace() in modules:
            ...
            action_classes.append(
                {
                    'class': class_,
                    'name': "%s.%s" % (cls.get_action_namespace(),
                                       action_name),
